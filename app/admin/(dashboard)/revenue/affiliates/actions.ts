...
import { getAffiliate360, generateAffiliateCode, WORK_CREDIT_RATE, CASH_RATE, type Affiliate360 } from "@/lib/admin/affiliates";
...
export async function getAffiliateShelf(personId: string): Promise<Affiliate360 | null> {
  await requireAdmin();
  return getAffiliate360(personId);
}
...
// Company-scoped when the person is linked to a company in the CRM, else
// company-less (Referrals-only). No-ops if a membership already exists.
async function ensurePortalAllowlist(personId: string, invitedBy: string): Promise<void> {
  ...
  const { data: link } = await companyOs
    ...
    .limit(1)
    .maybeSingle();
  const companyId = (link?.company_id as string | null) ?? null;

  await companyOs.from("portal_members").insert({
    person_id: personId,
    company_id: companyId,
    role: companyId ? "member" : "affiliate",
...
export async function sendAffiliateInvite(personId: string): Promise<Result> {
  ...
  if (existingAuth) {
    ...
    await sendTransactionalEmail({
      ...
      html: `<p>Here is your sign-in link for the Edge8 Client Portal:</p><p><a href="${data.properties.action_link}">Sign in</a></p>`,
    });
