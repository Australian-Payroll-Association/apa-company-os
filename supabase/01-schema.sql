...
--
-- Payroll 360 Discovery tool tables — applied to the live database via the
-- Supabase SQL Editor on 2026-09-01 (no direct psql/pg_dump access from this
-- environment to regenerate this file from a real dump, so appended here
-- rather than interleaved alphabetically like the rest of this file; content
-- is identical to supabase/features/discovery/00-schema.sql). Placed after
-- every other table so the companies/people/documents FK targets already
-- exist by the time these run in a fresh build.
--
